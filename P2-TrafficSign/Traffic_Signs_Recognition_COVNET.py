...
for j, option in enumerate(test['features']):
    ...
    ##Transform image to generate new image.
    tempTestFeature = transform_image(tempTestFeature, 20, 10, 5)
...
def plot_images(images, labels, cls_pred=None, rect_coords=None, reshape=True):
    ...
    ######### FIX TO NOT PLOT IMAGES IN CONSOLE RUN
    return
    #########
...
def conv_net(x, weights, biases, keep_prob):
    ...
    fc1 = tf.contrib.layers.flatten(conv1) ###conv3
...
feed_dict_test = {x: test["input-features-noresize"],
                  ...
                  keep_prob:1.0}
print("feed_dict_test Length= " , len(feed_dict_test[x]))
...
def optimize(num_iterations):
    for i in range(num_iterations):
        ...
        for batch_i in batches_pbar:
            ...
            #print ("batch_start=" , batch_start, "-", (batch_start + batch_size))
            ...
            if not batch_i % log_batch_step:
                ...
                print("Calculating training_accuracy_local")
                training_accuracy_local = session.run(accuracy, feed_dict=feed_dict_train)
                print("Calculating validation_accuracy")
                ...
                #",training_accuracy_full=", str(training_accuracy_full),      
