    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
			...
			*/
			//cout << "********************** STARTING MPC **********************" << endl;

		    Eigen::VectorXd vXVechicle;
		    Eigen::VectorXd vYVechicle;

			coordinatesTransform(ptsx, ptsy, px, py, psi, vXVechicle, vYVechicle);

			auto coeffs = polyfit(vXVechicle, vYVechicle, 3);
			//cout << "coeffs=" << coeffs << endl;
			double y_eval = polyeval(coeffs, 0.0);
			double cte = y_eval;
			double epsi =  -atan(coeffs[1]);

			Eigen::VectorXd state(6);
			//state << 0.0,0.0,0.0,v,cte,epsi; //px,py,psi,v,cte,epsi; // OLD STATE
			...
			for(int w=0; w<70; w++) {
				next_x_vals.push_back(w);
				next_y_vals.push_back(polyeval(coeffs, w)); //polyeval(coeffs, w)
			}
			//cout << "MPC 7" << endl;
