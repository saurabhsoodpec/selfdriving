...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	for (int i = 0; i < solution.x.size(); i++) {
		solutionsArr.push_back(solution.x[i]);
	}
...
/*!
 * Get the X co-ordinate trajectory
 */
vector<double> MPC::getXs() {
  return vector<double>(solutionsArr.begin() + x_start, solutionsArr.begin() + (x_start + N));
}

/*!
 * Get the Y co-ordinate trajectory
 */
vector<double> MPC::getYs() {
  return vector<double>(solutionsArr.begin() + y_start, solutionsArr.begin() + (y_start + N));
}
